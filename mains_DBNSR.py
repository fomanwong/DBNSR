...
def main():
    ...
    train_parser.add_argument("--batch_size", type=int, default=32, help="batch size,  32-x4/12-x8")
    train_parser.add_argument("--epochs", type=int, default=20, help="epochs, default set to 20")
    train_parser.add_argument("--n_feats", type=int, default=180, help="n_feats, default set to 180")
    train_parser.add_argument("--n_blocks", type=int, default=6, help="n_blocks, default set to 6")
    train_parser.add_argument("--n_subs", type=int, default=16, help="n_subs, 8-HT")
    ...
    train_parser.add_argument("--dataset_name", type=str, default="CK4",
                              help="dataset_name, default set to dataset_name")
    train_parser.add_argument("--model_title", type=str, default="HB",
...
def train(args):
    ...
    if args.dataset_name == 'HT4':
        train_path = '/HT/x4/trains/'
        eval_path = '/HT/x4/val/'
        test_data_dir = '/HT/x4/tests/'
    elif args.dataset_name == 'CK4':
        train_path = '/CK/x4/trains/'
        eval_path = '/CK/x4/val/'
        test_data_dir = '/CK/x4/tests/'
    elif args.dataset_name == 'PC4':
        train_path = '/PC/x4/trains/'
        eval_path = '/PC/x4/val/'
        test_data_dir = '/PC/x4/tests/'
    ...
    net = HBSR(n_subs=args.n_subs, n_ovls=args.n_ovls, n_colors=colors, n_blocks=args.n_blocks, n_feats=args.n_feats, n_scale=args.n_scale, res_scale=0.1,
               use_share=True)
    ...
    start_epoch = 0
    if resume:
        pt_path = r'.\trained_model\.pth'
        if os.path.isfile(pt_path):
            print("=> loading checkpoint '{}'".format(pt_path))
    ...
    current_time = time.strftime("%a_%b_%d_%H%M%S_%Y")
    writer = SummaryWriter('runs/' + model_title + '_' + current_time)
    ...
    for e in range(start_epoch, args.epochs):
        ...
        for iteration, (x, lms, gt) in enumerate(train_loader):
            ...
            loss = my_loss(y, gt) + 0.1 * (cal_sam(y, gt)) + my_loss(rec_input, x) + 0.1 * (
                cal_sam(x, rec_input))  # my_loss(y, gt)
    ...
    save_dir = './out/'+ model_title + '.npy'
    np.save(save_dir, output)
    print("Test finished, test results saved to .npy file at ", save_dir)
    print(indices)
    current_time1 = time.strftime("%a_%b_%d_%H%M%S_%Y")

    QIstr = './out/'+ model_title + '_' + current_time1 + ".txt"
