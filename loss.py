...
def cal_sid(Itrue, Ifake):
    ...
    for b in range(batch):
        for r in range(m):
            value = torch.sum(((Itrue[b, r, :] / torch.norm(Itrue, p=1, keepdim=True)) - (
                        Ifake[b, r, :] / torch.norm(Ifake, p=1, keepdim=True))) * (torch.log10(
                Itrue[b, r, :] / torch.norm(Itrue, p=1, keepdim=True) + 1e-3) - torch.log10(
                Ifake[b, r, :] / torch.norm(Ifake, p=1, keepdim=True) + 1e-3)))
            sum = sum + value
